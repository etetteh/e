...
def get_input_config(key):
    ...
    cfgs.experiment_name = st.text_input("Experiment Name",
                                         help="This command allows you to specify the name of the MLflow experiment. "
                                              "It helps organize and categorize the experimental runs.",
                                         key=f"exp_{key}")
    cfgs.dataset = st.text_input("Dataset Path",
                                 help="Use this command to provide the path to the dataset directory or the name of a "
                                      "HuggingFace dataset. It defines the data source for model training and "
                                      "evaluation.",
                                 key=f"dataset_{key}")
    cfgs.dataset_kwargs = st.text_input("Dataset Kwargs JSON Path",
                                        help="If needed, you can use this command to point to a JSON file containing "
    ...
    cfgs.hflip = st.number_input("Horizontal Flip Probability", value=0.5,
                                 help="Define the probability of randomly horizontally flipping the input data.",
                                 key=f"hflip_{key}")
    ...
    optimizers = ["lion", "madgradw", "adamw", "radabelief", "adafactor", "novograd", "lars", "lamb", "rmsprop", "sgdp",
                  "Other"]
    ...
    cfgs.grayscale = st.toggle("Use Grayscale Images",
                               help="If needed, use this flag to indicate that grayscale images should be used during "
                                    "training.",
    ...
    cfgs.avg_ckpts = st.toggle("Enable Checkpoint Averaging",
                               help="When enabled, this flag triggers checkpoint averaging during training. It helps "
                                    "stabilize the training process.",
                               key=f"avg_ckpts_{key}")

    cfgs.fgsm = st.toggle("Enable FGSM Adversarial Training",
                          help="This flag allows you to enable FGSM (Fast Gradient Sign Method) adversarial training "
    ...
    cfgs.cutmix = st.toggle("Enable Cutmix",
                            help="Enable cutmix augmentation, which combines patches from different images to create "
    ...
    with st.expander("More configurations"):
        cfgs.num_ckpts = st.number_input("Number of Checkpoints", min_value=1, value=1,
                                         help="Set the number of best checkpoints to save when checkpoint averaging "
                                              "is active. It determines how many checkpoints are averaged.",
                                         key=f"num_ckpts_{key}")
        cfgs.mag_bins = st.number_input("Magnitude bins", value=31,
                                        help="Set the number of magnitude bins for augmentation-related operations.",
                                        key=f"mag_bins_{key}")
        ...
        cfgs.epochs = st.number_input("Number of Epochs", value=100,
                                      help="Set the number of training epochs, determining how many times the entire "
                                           "dataset will be iterated.",
                                      key=f"epochs_{key}")
        ...
        cfgs.max_lr = st.number_input("Maximum Learning Rate", value=0.1,
                                      help="Set the maximum learning rate when using cyclic learning rate scheduling.",
                                      key=f"max_lr_{key}")
        cfgs.eta_min = st.number_input("Minimum Learning Rate", value=1e-4,
                                       help="Define the minimum learning rate that the scheduler can reach.",
                                       key=f"eta_min_{key}")
        cfgs.lr = st.number_input("Initial Learning Rate", value=0.001,
                                  help=" Specify the initial learning rate for the optimizer.",
                                  key=f"lr_{key}")
        cfgs.step_size = st.number_input("Step Size", value=30,
                                         help="Set the step size for learning rate adjustments in certain scheduler "
                                              "strategies.",
                                         key=f"step_size_{key}")
        cfgs.warmup_epochs = st.number_input("Warmup Epochs", value=5,
                                             help="Specify the number of epochs for the warmup phase of learning rate "
                                                  "scheduling.",
                                             key=f"warmup_epochs_{key}")
        cfgs.warmup_decay = st.number_input("Warmup Decay", value=0.1,
                                            help="Set the decay rate for the learning rate during the warmup phase.",
                                            key=f"warmup_decay_{key}")
        cfgs.gamma = st.number_input("Gamma", value=0.1, help="Set the gamma parameter used in certain learning rate "
                                                              "scheduling strategies.",
                                     key=f"gamma_{key}")

        cfgs.t0 = st.number_input("First Restart Iterations", value=5,
...
if __name__ == "__main__":
    ...
    with tab3:
        ...
        tune_cfgs.cpus_per_trial = st.number_input("Number of CPUs per Trial", value=8, help="Number of CPUs per trial")
        tune_cfgs.gpus_per_trial = st.number_input("Number of GPUs per Trial", value=0, help="Number of GPUs per trial")
        tune_cfgs.search_algo = st.text_input("Hyperparameter Search Algorithm", value="bohb",
                                              help="Hyperparameter search algorithm")

        scheduler = st.radio(
            "Select your optimization algorithm (AKA scheduler)",
            ["Use ASHA", "Use PBT", "Use PB2"],
            captions=["Use Async Successive Halving (ASHA) algorithm",
    ...
    with tab4:
        ...
        infer_args.dataset_kwargs = st.text_input("Dataset Kwargs (Path to JSON file)", help="If necessary, you can "
                                                                                             "provide the path to a "
                                                                                             "JSON file containing "
                                                                                             "keyword arguments ("
                                                                                             "kwargs) specific to a "
                                                                                             "HuggingFace dataset.")
        infer_args.crop_size = st.number_input("Inference Image Crop Size", value=224, help="Define the size to which "
                                                                                            "input images will be "
                                                                                            "cropped during inference.")
        infer_args.val_resize = st.number_input("Inference Image Resize", value=256, help="Specify the size to which "
                                                                                          "validation images will be "
    ...
    with tab5:
        ...
        args.model_output_dir = st.text_input("Model Output Directory", help="Specifies the output directory where "
                                                                             "the model is contained")

        args.dataset = st.text_input("Dataset", help="Use this command to provide the path to the dataset directory "
                                                     "or the name of a HuggingFace dataset. It defines the data "
                                                     "source for the explanation")
        args.dataset_kwargs = st.text_input("Dataset kwargs", help="If necessary, you can provide the path to a JSON "
                                                                   "file containing keyword arguments (kwargs) "
                                                                   "specific to a HuggingFace dataset.")

        args.crop_size = st.number_input("Image Crop Size", value=224, help="Define the size to which input images "
                                                                            "will be cropped.")
        args.batch_size = st.number_input("Batch Size", value=128, help="Define the batch size for both training and "
                                                                        "evaluation stages.")
        args.num_workers = st.number_input("Number of Workers", value=8, help="Specify the number of workers for "
        ...
        args.dropout = st.number_input("Dropout", value=0.2, help="Define the dropout rate for the classifier head of "
                                                                  "the model.")
        args.grayscale = st.toggle("Use Grayscale Images", value=False, key="inference", help="Use this flag if you "
                                                                                              "want to use grayscale "
                                                                                              "images ")
        args.feat_extract = st.toggle("Enable Feature Extraction", value=True, help="By including this flag, you can "
                                                                                    "enable feature extraction during"
                                                                                    " training, which is useful when "
